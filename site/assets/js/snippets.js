...
(() => {
  ...
  const MCRipple = (() => {
    ...
    const roleEls = ['[data-bs-toggle="tab"]']
    ...
    const addRippleSpanElement = (element, unbounded) => {
      ...
      rippleElement.appendChild(element)
      applyRippleEffect(element, rippleElement, unbounded)
    }
    ...
    const rippleSurface = () => {
      const buttonDefaults = getElements(`[class^="btn"]:not(label):not(input):not(div):not(${iconButtonEls.map(cls => `.${cls}`).join(', ')})`)
